...
def main():
    ...
    parser.add_argument(
        ...
        '--subject',
        nargs='+',
        help='subject start index and end index',
        type=int
    )
    parser.add_argument(
        '--flip',
        action='store_true',
        help='Do the flipping'
    )
...
def run(args):
    ...
    combos = get_combos(product(local_config['subjects'] if args.subject is None else list(range(args.subject[0], args.subject[1])),
    ...
    def extract(input_path, output_path, combo, feature_list):
        ...
        if args.flip:
            data = abs(data)
            output_path = output_path + '-flip'

        if args.filter is 'lowpass':
        ...
        if args.downsample > 1:
            data = downsample(data, step=args.downsample)
            output_path = output_path + '-downsample%d' % (args.downsample)
        ...
        for feature_name in feature_list:
            ...
            try:
                ...
                log.info('%s occurs error' % out_path)
    for combo in tqdm(combos,
                      desc='{d}-subject-{b:d}~{e:d}'.format(d=args.dataset, b=args.subject[0], e=args.subject[1]),
                      leave=False):
        #t = threading.Thread(target=extract,
        #                     args=(args.input, args.output, combo, args.featurelist,))
        #t.start()
        extract(args.input, args.output, combo, args.featurelist)
