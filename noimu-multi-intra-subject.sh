...
python -m sigr.train exp --log log --snapshot model \
    --root .cache/multi-no-imu-one-fold-intra-subject-flip-ninapro-db1-win-20-stride-1/universal-one-fold-intra-subject \
...
for i in $(seq 0 26); do
    python -m sigr.train exp --log log --snapshot model \
    --root .cache/multi-no-imu-one-fold-intra-subject-flip-ninapro-db1-win-20-stride-1/one-fold-intra-subject-$i \
    --params .cache/multi-no-imu-one-fold-intra-subject-flip-ninapro-db1-win-20-stride-1/universal-one-fold-intra-subject/model-0028.params \
    --batch-size 1000 --decay-all --dataset ninapro-db1 \
    --num-filter 64 \
    --num-epoch 28 --lr-step 16 --lr-step 24 --snapshot-period 28 \
    --balance-gesture 1 \
