...
print('Biopatrec DB3 Singlestream')
print('===========')
...
with Context(parallel=True, level='DEBUG'):
    acc = one_fold_intra_subject_eval.vote_accuracy_curves(
        [Exp(dataset=Dataset.from_name('biopatrec-db3'),
             Mod=dict(num_gesture=10,
                      ...
                      params='.cache/single-biopatrec-db3-3FSsigimg-win-300-stride-100/one-fold-intra-subject-fold-%d/model-0028.params'))],
        folds=np.arange(8),
