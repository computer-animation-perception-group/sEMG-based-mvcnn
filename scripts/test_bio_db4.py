...
print('Biopatrec DB4 Singlestream')
print('===========')


semg_row = 128 # sigimg length
semg_col = 1
...
with Context(parallel=True, level='DEBUG'):
    acc = one_fold_intra_subject_eval.vote_accuracy_curves(
        [Exp(dataset=Dataset.from_name('biopatrec-db4'),
             Mod=dict(num_gesture=8,
                      ...
                      params='.cache/single-biopatrec-db4-3FSsigimg-win-300-stride-100/one-fold-intra-subject-fold-%d/model-0028.params'))],
        folds=np.arange(8),
