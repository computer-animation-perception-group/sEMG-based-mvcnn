...
def main():
    ...
    parser.add_argument(
        '--fusion',
        choices=['single', 'single_with_imu', 'multi_no_imu', 'multi_with_imu'],
        help='Fusion type',
        default='single')
    parser.add_argument(
        '-c',
...
def run(args):
    ...
    if args.fusion == 'single':
        ...
        semg_col = []
        num_ch = dataset_config['num_chs']
        num_ch.append(dataset_config['window'])
        ...
        acc = eval_type.vote_accuracy_curves(
            [Exp(dataset=Dataset.from_name(args.dataset),
                 Mod=dict(num_gesture=num_gesture,
                          ...
                          params='.cache/adabn-multi-inter-subject-flip-ninapro-db5-with-imu-win-40-stride-20/one-fold-intra-subject-%d/model-0028.params'))],
