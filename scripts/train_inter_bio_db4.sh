# single inter-subject 50 gpu-1 
for i in $(seq 0 3); do
    python -m sigr.train exp --log log --snapshot model \
        --root .cache/single-inter-subject-biopatrec-db4-3FSsigimg-win-300-stride-100/4-fold-inter-subject-$i \
...
#for i in $(seq 0 7); do
#    python -m sigr.train exp --log log --snapshot model \
#    --root .cache/single-adabn-inter-subject-biopatrec-db4-3FSsigimg-win-300-stride-100/one-fold-intra-subject-fold-$i \
...
for i in $(seq 0 3); do
    python -m sigr.train exp --log log --snapshot model \
        --root .cache/multi-inter-subject-biopatrec-db4-3FSsigimg-win-300-stride-100/4-fold-inter-subject-$i \
        --batch-size 800 --decay-all --dataset biopatrec-db4 \
        --num-filter 64 \
        --num-epoch 28 --lr-step 16 --lr-step 24 --snapshot-period 28 \
        --balance-gesture 1 \
...
#for i in $(seq 0 7); do
#    python -m sigr.train exp --log log --snapshot model \
#    --root .cache/multi-adabn-inter-subject-biopatrec-db4-3FSsigimg-win-300-stride-100/one-fold-intra-subject-fold-$i \
