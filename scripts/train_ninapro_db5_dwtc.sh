...
python -m sigr.train exp --log log --snapshot model \
    --root .cache/Feature_selection/DWTC-ninapro-db5-featuresigimg-win-40-stride-20/universal-one-fold-intra-subject \
...
for i in $(seq 0 9); do
    python -m sigr.train exp --log log --snapshot model \
    --root .cache/Feature_selection/DWTC-ninapro-db5-featuresigimg-win-40-stride-20/one-fold-intra-subject-$i \
    --params .cache/Feature_selection/DWTC-ninapro-db5-featuresigimg-win-40-stride-20/universal-one-fold-intra-subject/model-0028.params \
    --batch-size 1000 --decay-all --dataset ninapro-db5 \
    --num-filter 64 \
    --num-epoch 28 --lr-step 16 --lr-step 24 --snapshot-period 28 \
    --balance-gesture 1 \
