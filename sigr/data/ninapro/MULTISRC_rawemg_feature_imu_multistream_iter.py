...
class Feature_RawSEMG_IMU_Multistream_SigImg_Iter_v1(mx.io.NDArrayIter):
...
#        self.feature_list = kargs.pop('feature_list')
        ...
        super(Feature_RawSEMG_IMU_Multistream_SigImg_Iter_v1, self).__init__(*args, **kargs)
...
#
        ...
        self.channel = [self.num_channel[0]]
        for i in range(3):
            channel = self.channel[i] + self.num_channel[i + 1]
...
#        res = res.reshape(res.shape[0], 1, res.shape[1], res.shape[2])
    ...
    def _get_actimg(self, data, sigimg_index):

        from ... import Context
        import joblib as jb
        res = []

        for amp in Context.parallel(jb.delayed(_get_actimg_aux)(sample, sigimg_index) for sample in data):
            res.append(amp[np.newaxis, ...])
        res = np.concatenate(res, axis=0)
#        res = res.reshape(res.shape[0], 1, res.shape[1], res.shape[2])
        res = res.reshape(res.shape[0], res.shape[1], res.shape[2], -1)
...
#             print res[0].shape
             feature_data = res[0].asnumpy() if isinstance(res[0], mx.nd.NDArray) else res[0]
             imu_data = res[1].asnumpy() if isinstance(res[1], mx.nd.NDArray) else res[1]
             ...
             data_1 = feature_data[:, 0:self.channel[0], :]
             data_2 = feature_data[:, self.channel[0]:self.channel[1], :]
             data_3 = feature_data[:, self.channel[1]:self.channel[2], :]
             data_4 = imu_data
...
#             data_4 = temp_data[:,82:89,:]
             ...
             res_.append(self._get_actimg(data_4, sigimg_index))
...
#             print len(self.num_channel)
    ...
    def reset(self):
        self._reset()
        super(Feature_RawSEMG_IMU_Multistream_SigImg_Iter_v1, self).reset()
...
def get_act_img(data, sigimg_index):
    epison = 1e-5
    signal_img = get_sig_img(data, sigimg_index)
    f = np.fft.fft2(signal_img)
    fshift = np.fft.fftshift(f)
    magnitude_spectrum = 20 * np.log(epison + np.abs(fshift))

    return magnitude_spectrum

def _get_actimg_aux(data, sigimg_index):
    return np.transpose(get_act_img(data.T, sigimg_index))
